...
import { setCourseTypeIndex, setCreateCourseTypeModalOpen, setDeleteCourseTypeModalOpen, setUpdateCourseTypeModalOpen } from "../Redux/CourseSlice";
                        ...
                        courseType.map((item: CourseType, index: number) => {
                            return (
                                <div key={index} className={`flex w-full p-2 ${index % 2 != 0 && "bg-slate-50"}`}>
                                    ...
                                    <div className="w-full flex items-center justify-between pl-2 pr-2 gap-2 sm:pr-4">
                                        <div className="w-[80%]">
                                            <p>{item.coursetype}</p>
                                        </div>
                                        <button onClick={() => {
                                            dispatch(setCourseTypeIndex(index))
                                        ...
                                        <button onClick={() => {
                                            dispatch(setCourseTypeIndex(index))
                                            dispatch(setDeleteCourseTypeModalOpen())
